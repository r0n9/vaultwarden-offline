...
当前生成器是基础版，后续按 Bitwarden 的功能逐步补充：

**密码**
- 生成参数面板：长度、字符集、各类最少数量在 UI 中可调（现为「强密码 / 密码短语」两个预设按钮）
- 熵估算与强度指示

**密码短语**
- EFF 官方词表（当前为自制常见单词表——EFF 词表是公开文本但需联网获取，与零网络承诺冲突；
  可行路径是把词表作为构建期资源随包发布，运行期不联网）
- 词表选择（EFF long / short）、大小写模式（每词大写 / 全小写 / 全大写）、数字后缀开关的参数化 UI

**用户名**
- 随机字串型
- 子地址 / catchall（`user+tag@domain`，可离线实现）
- 转发邮箱别名（SimpleLogin / AnonAddy 等，**需联网，明确不做**）

**通用**
- 生成历史（加密存储、可清空）—— 已在 P1 列表
- 生成器可嵌入任意自定义字段

#### 待开发功能

**目录（文件夹）**
- 列表页目录展示与切换（当前只能在设置 tab 管理文件夹，列表页没有目录入口）
- 列表页新增目录按钮（不用进设置）
- 条目编辑时的目录选择（当前只有「无文件夹 / 现有文件夹」下拉）

**站点 favicon 图标自动更新**
- 当前条目图标为本地生成的首字母色块（零网络，见 Phase 4 说明）
- 待实现：展示站点真实 favicon 并随站点更新缓存
- ⚠️ 约束：直接请求站点 favicon（`/favicon.ico` 等）会违反零网络承诺。
  可行路径需评估：构建期/导入期附带图标数据、用户显式授权后按需拉取、
  或维持本地生成并仅做本地缓存管理。
