...
- [x] **Phase 0** 项目脚手架 —— 三路构建（popup / SW / content script）、零网络校验
- [x] **Phase 1** 加密核心 —— EncString、AES-CBC-HMAC、PBKDF2/Argon2id、密钥层级
- [x] **Phase 2** 存储与锁定 —— 8 种条目类型、品牌类型明文/密文分离、字段规格表、
  四种锁定触发、失败指数退避、主密码策略（≥8 位含字母数字）
- [x] **Phase 3** 导入导出 —— JSON（明文/密码保护）与 CSV 双向互通，真实数据无损往返
- [x] **Phase 4** 条目管理 —— 列表/详情/编辑、搜索筛选、文件夹、回收站、收藏、密码历史
- [x] **Phase 5** 自动填充引擎 —— 采集、判定、执行、右键菜单、保存提示条、快捷键、站点匹配
- [x] **Phase 6** 生成器 / TOTP / PIN —— 见下方明细

**Phase 6 明细**

- 密码生成（长度/字符集/最少数量/剔除歧义，无偏随机）
- 密码短语（词数/分隔符/首字母大写/数字后缀可配）与用户名生成
- TOTP 动态码（RFC 6238 官方向量验证，含 Steam 5 字符码）
- PIN 解锁（4-12 位数字字母，PIN/主密码切换，共用解锁节流）
- 新增菜单支持「新增目录」
- 站点 favicon 自动获取与缓存（零网络承诺的唯一例外）

**待办（暂缓）**

- 内联菜单浮层（输入框聚焦时的下拉选择）—— 工作量最大的一块，
  需要时再实现（参考 Bitwarden 的 overlay 模块，约 3600 行）

**明确不做**

- 账户注册登录 · 2FA/SSO · 同步 · Send · 组织共享 · HIBP 泄露查询 · 钓鱼检测 ·
  遥测 · 计费 · 转发邮箱别名（SimpleLogin / AnonAddy 等，需联网）
